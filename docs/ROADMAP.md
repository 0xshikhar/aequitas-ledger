...
- **Status:** ✅ Done (production-shaped protocol)
- **Resolution — all five items closed, protocol redesigned:**
  1. **Follower group commit preserved.** Data records buffer until the primary's `BatchCommit` marker closes the batch, then ONE `AppendBatch` + Sync persists them and only after durability are they applied to in-memory state (the follower WAL is always ≥ its in-memory view, so promotion replays complete history). Proof: `TestFollowerPreservesGroupCommit` — 152 data records with **4 commit markers** (previously 152 fsyncs).
  2. **Exponential backoff + circuit breaker.** Failures back off 100ms→5s (2^n); after `BreakerLimit` (8 default, tunable) consecutive failures the follower performs a full re-sync — fresh state + `WAL.Reset()` — instead of resuming a possibly-divergent position. Stale-term refusals are classified separately (`errStalePrimary`) and deliberately bypass the breaker: fencing is protective, not state uncertainty (the first draft tripped the breaker and wiped valid state while fenced — caught by the fencing test).
  3. **Leader-term fencing.** Every frame is `[term:8][LSN:8][type:1][len:4][payload][crc32]` (term inside the CRC). The primary persists its term (`replication.term`, initialized 1, bumped once per promotion via `BumpTerm` in `PromoteToPrimary`); the follower persists its last-accepted term and refuses streams/frames below it. Proof: `TestFollowerFencesStalePrimary` — a term-1 stale server cannot touch a follower holding term-2 state.
  4. **Streaming tail instead of reconnect re-scans.** New `wal.StreamReader` delivers committed records incrementally (durable-bound: closed segments fully, the live segment only through its synced offset), and connections now STAY OPEN tailing the WAL at a 5ms poll — no re-scan per reconnect, and followers never see records the primary could lose in a crash. Regression proof: `TestStreamReaderLongLivedKeepsDelivering` (a reader stuck permanently at caught-up after its first idle pass — the live-segment advance bug), `TestStreamReaderHidesUnsyncedTail`, `TestStreamReaderDeltaAcrossSegments`.
  5. **Follower restart recovery.** On Start the follower replays its own WAL to rebuild state and the true applied position (commit-LSN = max data LSN + 1 in the primary's contiguous space) — a restart no longer re-streams duplicate history over an existing WAL, which previously corrupted post-promotion balances. A follower whose WAL cannot be replayed refuses to start (fail-safe). Proof: `TestFollowerRestartRecoveryAndResync` — restart resumes by delta with zero duplicate records; the breaker then rebuilds everything from LSN 1.
  Plus **metrics/HEAD (the readiness signal):** the primary sends periodic `RecordTypeHead` announcements (stream-only record type); the follower exposes `LastLSN`/`Lag`/`Connected` gauges + a reconnect counter, the primary an active-followers gauge. `/readyz` (C0.5) takes a readiness function so a follower can gate on its own catch-up.
- **Proof:** all four replication tests + the three streamer tests pass under `-race`; the whole suite is green.
...
- **Status:** ✅ Done
- **Resolution:** Implemented full two-phase hold lifecycle: `FlagPending` reserves funds from debit account `AvailableBalance` into `PendingDebits` and credit account `PendingCredits`; `FlagPostPending` settles the hold (with full settlement if `Amount == 0` or partial settlement, rejecting over-settlement with typed `core.ErrOverTransfer`); `FlagVoidPending` releases the hold. Monotonic logical clock `l.clock` tracks engine batches and auto-voids expired holds deterministically. Exposed across proto stubs, gRPC, REST (`POST /v1/transfers`), and CLI (`ledger-cli transfer create --flags --timeout`).
- **Proof:** `tests/integration/two_phase_query_test.go` (`TestTwoPhaseHoldLifecycle`, `TestTwoPhaseHoldVoid`, `TestTwoPhaseHoldExpiryOnLogicalClock`).

### D1.3 — Linked transfers (chains)
- **Status:** ✅ Done
- **Resolution:** Implemented `TransferFlagLinked` (flag 8) semantics with `batchValidator` in `internal/engine/transfers.go`. A linked chain commits all legs atomically or rolls back all legs without writing any WAL record. Later legs in a chain observe staged balance updates of earlier legs (enabling atomic multi-leg escrow/settlement). If any transfer in the chain fails validation, all other transfers in that chain receive typed `core.ErrLinkedChainFailed` referencing the failed transfer ID and index. Chains extending past the batch boundary are rejected with `core.ErrLinkedChainOpen`. Unlinked transfers in the same batch execute and commit independently.
- **Proof:** `tests/integration/linked_transfers_test.go` (`TestLinkedTransfersMultiLegSuccess`, `TestLinkedTransfersFailureRollsBackAllLegs`, `TestLinkedTransfersDanglingOpenChain`, `TestMixedBatchIsolation`).

### D1.4 — Rich schema: ledger, code, user_data
- **Status:** ✅ Done
- **Resolution:** Expanded domain model and serialization codecs byte-for-byte:
  - `core.Account` (128 bytes): `ID(16) + Currency(4) + Ledger(4) + Code(2) + pad(2) + Flags(4) + UserData128(16) + PostedDebits(16) + PostedCredits(16) + PendingDebits(16) + PendingCredits(16) + reserved(16) = 128`. Naturally aligned, power of two, enforced by compile-time guard `AccountStructSize = 128`.
  - `core.Transfer` (144 bytes): `ID(16) + Debit(16) + Credit(16) + Amount(16) + Idemp(32) + UserData(16) + TS(8) + Timeout(8) + Ledger(4) + Code(2) + pad(2) + Flags(4) + pad(4) = 144`. Enforced by compile-time guard `TransferStructSize = 144`.
  - Multi-tenant isolation enforced in engine: `debit.Ledger == credit.Ledger == transfer.Ledger`, returning typed `core.ErrLedgerMismatch` on violation.
  - Multi-tenant conservation enforced in `internal/audit/audit.go`: verifies $\sum \text{Credits} - \sum \text{Debits} == \text{InjectedCredits}$ independently per `Ledger uint32`.
  - Upgraded snapshot format to magic header `LEDGER03` with 128-byte account serialization.
  - Exposed across proto stubs, gRPC, REST, and CLI (`-ledger`, `-code`, `-user-data`).
- **Proof:** `tests/unit/codec_test.go` (zero-alloc round-trip and struct size tests) and `tests/integration/multi_ledger_test.go` (`TestMultiTenantLedgerIsolation`).

### D1.5 — Query APIs (point lookup & paginated history)
- **Status:** ✅ Done
- **Resolution:** Added zero-data-race loop-hop read channels (`readTransferQueue`, `readAccountTransfersQueue`) routed strictly through `EventLoop`. Implemented `GetTransfer(id)` point lookups and `GetAccountTransfers(account_id, limit, after_id)` with cursor-based pagination. Exposed across proto stubs, gRPC, REST (`GET /v1/transfers/{id}`, `GET /v1/accounts/{id}/transfers`), and CLI (`ledger-cli transfer get`, `ledger-cli transfer list`).
- **Proof:** `tests/integration/two_phase_query_test.go` (`TestQueryAPITransfersAndHistory`) and `tests/integration/rest_test.go`.

### D1.6 — Timestamp discipline
- **Status:** ✅ Done
- **Resolution:** Batch-synchronous timestamp discipline implemented in `internal/engine/loop.go` (`processBatch`). The system clock is sampled once at the batch boundary and incremented strictly monotonically if `now <= l.clock`. All transfers in the batch share this exact nanosecond timestamp, eliminating per-transfer clock drift. The timestamp is durably committed to the WAL; followers, audit tools, and crash-recovery loops replay the exact same timestamp without clock re-evaluation.
- **Proof:** `tests/integration/batch_timestamp_test.go` (`TestBatchSynchronousTimestampsAndMonotonicity`).
...
- **Status:** ✅ Done
- **Resolution:** Preallocate segments to 64 MiB at creation using `unix.Fallocate` (`FALLOC_FL_KEEP_SIZE`) on Linux and `fcntl(F_PREALLOCATE)` with `F_ALLOCATECONTIG`/`F_ALLOCATEALL` on macOS, eliminating runtime metadata locks and filesystem fragmentation. Implemented 4KB-aligned buffer allocator (`NewAlignedBuffer`), 4KB block direct I/O buffering (`directIOBuffer`), and Linux `unix.O_DIRECT` opener with transparent fallback to buffered I/O on dev/macOS platforms.
- **Proof:** `tests/unit/wal_prealloc_test.go` (`TestAlignedBuffer`, `TestSegmentPreallocationAndDirectIO`).
...
- **Status:** ✅ Done
- **Resolution:** `benchmark_report.md` fully rewritten around reproducible, committed benchmarks: gRPC unary (211 TPS) vs batch-8192 (415k TPS after S2.1 — the 200k–500k claim is now *measured*, and the report says which clients hit it), the engine batch-size curve 1/128/1024/8192 with per-transfer allocations, the S2.1 before/after allocation table, read-latency-under-load budgets, recovery timing (with the honest finding that at 163k records the snapshot path was NOT faster than full replay — both sub-ms — and the snapshot's value is bounding O(history) growth), and five explicit caveats (single node, fsync-bound, bufconn, unary-is-the-baseline, CI floors). pprof evidence referenced from `docs/learning/assets/`. Every number is regenerated by a committed benchmark.
...
- **Status:** ✅ Done (core property; nightly long-run mode still open)
- **Resolution:** `TestCrashAtEveryByteOffset` (`tests/integration/crash_property_test.go`) — a deterministic workload (accounts + three multi-record transfer batches) is truncated at **every byte offset** (~1.1k cases). Verification uses an independent model: a frame parser written inside the test (not the wal package) defines batch boundaries, and a map-based simulator recomputes balances. Per offset it asserts: recovery applies exactly the batches whose commit fully preceded the cut (the property C0.17 violated), the watermark equals the last applied commit LSN, `NewLedger`-recovered balances match the model byte-for-byte, and the invariant holds. The test's own model initially had the `pending[:0]` aliasing bug (stored batch slices sharing one backing array) — a nice demonstration of why the model must be simple.
- **Still open:** a long-run nightly mode (10M random truncations) and multi-segment random-truncation sweeps.

### T3.2 — Fault-injecting WAL wrapper
- **Status:** ✅ Done
- **Resolution:** Implemented `FaultInjector` (`internal/wal/fault_injector.go`) providing bit flips across record headers, payloads, and CRC trailers, mid-frame torn writes, and garbage tail injection. Property test verifies recovery never crashes or panics, halts cleanly at the last valid batch commit boundary, truncates stale data, and guarantees non-corrupted account balance conservation.
- **Proof:** `tests/integration/fault_injection_test.go` (`TestFaultInjection_BitFlips`, `TestFaultInjection_TornWriteAndGarbageTail`).

### T3.3 — Replication adversarial tests
- **Status:** ✅ Done
- **Resolution:** Built an in-process chaotic TCP proxy (`chaoticProxy`) capable of injecting frame payload corruption, mid-stream socket termination, transmission delays, and stale leader terms. Proved follower rejects corrupted frames without data corruption, fences stale primaries, reconnects via exponential backoff circuit breaker, and resumes exactly from its local applied watermark.
- **Proof:** `tests/integration/replication_adversarial_test.go` (`TestReplicationAdversarial_CorruptionAndRecovery`, `TestReplicationAdversarial_SuddenDropAndResume`, `TestReplicationAdversarial_StaleTermRefused`).

### T3.4 — First-class invariant auditor
- **Status:** ✅ Done
- **Resolution:** `internal/audit` + `ledger-cli audit --wal <dir>`. The auditor re-implements the frame format, batch atomicity, LSN ordering, and conservation from scratch — it reuses only the payload codecs (the shared contract) and, unlike engine recovery, **heals nothing**: corrupt frames, count-mismatched batches, and torn tails are reported as findings, not truncated. Checks: CRC per frame, commit-count matching, strictly increasing LSNs, per-account non-negative balances, system-wide conservation against credits injected at account creation, idempotency-key uniqueness, and transfer-ID duplication. Exit code 1 on any violation. E2E proof: `TestAuditEngineWALPasses` — a real engine WAL (200 transfers + snapshot truncation) audits clean with exact record counts; planted-bug tests cover corrupted frames, torn tails, duplicate keys, and unknown-account transfers.
...
- **P5.3 Security (current Phase 14):** ✅ Done (Replication TLS & mTLS) — Added TLS/mTLS configuration into `config.Config` (`REPLICATION_TLS_ENABLED`, certs, keys, CA pool). Primary listener wrapped with `tls.NewListener`, follower dials with `tls.Dial` verifying server identity and presenting client certificates. Plaintext connections to the TLS replication listener fail cleanly. Verified in `tests/integration/replication_tls_test.go` (`TestReplicationTLS_EndToEndAndPlaintextRejection`, `TestReplicationMTLS_MutualAuthentication`).
...
- **2026-09-06 (C0.5 + C0.7 + C0.8):** Tier 0 is now fully closed. **C0.5**: the background checkpointer runs from `cmd/server` via `Ledger.StartCheckpointer` (first cycle synchronous → deterministic readiness; ticker thereafter), truncating WAL segments on every snapshot — production WALs stop growing without bound; `Ready()` + REST `/readyz` gate on recovery + first snapshot cycle; the WAL gained a mutex so the checkpointer's `TruncateBefore` is race-free against the writer loop, plus `Head()` exposing the durable synced offset. **C0.7**: `core.Balance` returns `ErrInvariantViolation` instead of panicking; invariant-broken accounts are rejected at creation (per item, engine + InitialAccounts); the invariant-violations metric is the never-fire alert; the panic grep gate now has an empty allowlist. **C0.8**: replication redesigned — follower group commit (4 commit markers for 152 records, was 152 fsyncs), exponential backoff + circuit breaker with full re-sync (`WAL.Reset()`), leader-term fencing with persisted terms (`BumpTerm` on promotion; stale refusals bypass the breaker), `wal.StreamReader` durable-bound streaming tail (no reconnect re-scans; three regression tests including a live-segment positioning bug), follower restart recovery from its own WAL (no duplicate history), and lag/connected/reconnect/active-follower metrics via HEAD announcements. Known limitation documented: a follower that applied records the primary later truncated (crash between record-fsync and commit-write on the primary) requires an operator-triggered re-sync — the term file does not catch same-term divergence. Remaining open: D1.2 (two-phase transfers), D1.5 (query API), S2.1/S2.4 (write path + honest bench report), T3.x (property tests + auditor), P5.3 (TLS).
- **2026-09-06 (S2.1 + S2.4 + T3.1 + T3.4):** Write path and verification suite landed. **S2.1**: single-buffer batch encoding + one sequential Write (frame-boundary-safe across rotations), append-codecs + loop scratch buffers, fdatasync on Linux, and the busy-spin replaced by a blocking idle select over all wakeup sources — batch-8192 23.4→12.9 ms (−45%), 3.01→1.01 allocs/transfer, idle CPU ~0%, GetAccount unregressed at 617 ns/0 allocs. **S2.4**: benchmark_report.md rewritten with reproducible numbers — gRPC unary 211 TPS vs batch-8192 415k TPS (~1,970×), the batch curve, allocation table, recovery timing with the honest finding that snapshot≈full-replay at 163k records (the snapshot's value is the asymptotic bound), and five caveats. **T3.1**: crash-at-every-byte-offset property test (~1.1k cases) with an independent frame parser and balance model — every offset recovers exactly the committed prefix. **T3.4**: internal/audit + `ledger-cli audit --wal` independent replayer (CRC, batch atomicity, LSN order, conservation, key/ID uniqueness) with planted-bug tests. One flake observed in the integration suite during a heavily loaded run; fresh runs pass — timing-sensitive tests (read latency, replication catch-up) are candidates for hardening if it recurs. Remaining open: D1.2/D1.5 (two-phase transfers, query API), S2.2 (direct I/O/preallocation), T3.2/T3.3 (fault injection, replication adversarial), P5.3 (TLS).
- **2026-09-07 (D1.2 + D1.5 + S2.2 + T3.2 + T3.3 + P5.3):** Closed all remaining core roadmap items. **D1.2**: Two-phase transfers with full reserve, partial/full post, void, and engine logical clock auto-expiry; proto/gRPC/REST/CLI integrated. **D1.5**: Thread-safe loop-hop read queues in `EventLoop` (0 data races) for `GetTransfer` and `GetAccountTransfers` with cursor pagination. **S2.2**: Segment preallocation to 64 MiB (`unix.Fallocate` with `FALLOC_FL_KEEP_SIZE` on Linux, `fcntl(F_PREALLOCATE)` on macOS) and 4KB-aligned buffer path with Linux `O_DIRECT` and dev fallback. **T3.2**: Fault injector injecting bit flips (header, payload, CRC), torn writes, and garbage tails; asserted clean recovery and balance conservation without silent corruption. **T3.3**: In-process chaotic TCP proxy injecting byte corruption, mid-stream connection drops, and stale term fencing. **P5.3**: TLS and mTLS support on the replication streaming listener and follower dialer. All unit and integration suites pass under `-race`.
